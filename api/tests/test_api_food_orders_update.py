...
class UpdateFoodOrderTest(TestCase):
    ...
    def setUp(self):
        ...
        self.food_order_update['food_id'] = self.food_order.food_id
    ...
    def test_update_food_order(self):
        ...
        total = self.food_order_update['price'] * self.food_order_update['quantity']
        price = self.food_order_update['price']
        self.assertEqual(response.data['price'], price)
        self.assertEqual(response.data['total'], total)
        self.assertEqual(response.data['quantity'], 10.0)
